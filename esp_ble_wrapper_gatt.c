...
#include "trace.h"
#include "esp_ble_wrapper_gap.h"
#include "esp_ble_wrapper_gatt.h"
#include "esp_ble_wrapper_config.h"
#include "esp_ble_wrapper_profile.h"
#include "esp_gatt_common_api.h"

static uint16_t g_mtu_size = ESP_GATT_DEF_BLE_MTU_SIZE;

static char *esp_ble_wrapper_gatt_event_to_string(esp_gatts_cb_event_t event);
static void esp_ble_wrapper_gatt_call_read_by_handle(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);
static void esp_ble_wrapper_gatt_call_write_by_handle(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);
static void esp_ble_wrapper_gatt_call_write_exec_by_handle(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);
...
{
    TRACE_B("BLE GATT Event: %s, gatts_if: %d", esp_ble_wrapper_gatt_event_to_string(event), gatts_if);

    switch (event)
    {
    case ESP_GATTS_REG_EVT:
    {
        s_gatt_service_t *service = esp_ble_wrapper_profile_get_by_app_id(param->reg.app_id);
        if (service)
        {
            TRACE_D("Found app-id: %d", service->app_id);
            esp_ble_wrapper_gap_config_adv_data();
            esp_ble_wrapper_gap_config_scan_rsp_data();
    ...
    {
        s_gatt_service_t *service = esp_ble_wrapper_profile_get_service_by_gatts_if(gatts_if);
    ...
    case ESP_GATTS_START_EVT:
    {
        ...
        s_gatt_service_t *service = esp_ble_wrapper_profile_get_service_by_gatts_if(gatts_if);
        s_gatt_char_t *char_to_add = esp_ble_wrapper_profile_get_characterstics_to_init(service);
        if (char_to_add)
        {
            ...
            // esp_ble_wrapper_gatt_print_characteristic(char_to_add);
            if (err)
            {
    ...
    case ESP_GATTS_ADD_CHAR_EVT:
    {
        ...
        s_gatt_service_t *service = esp_ble_wrapper_profile_get_service_by_gatts_if(gatts_if);
        s_gatt_char_t *char_initiating = esp_ble_wrapper_profile_get_initiating_characterstics(service);

        if (char_initiating)
        {
            char_initiating->handle = param->add_char.attr_handle;
            s_gatt_descr_t *desc_to_init = esp_ble_wrapper_profile_get_descriptor_to_init(char_initiating);
            if (desc_to_init)
            {
                ...
                // esp_ble_wrapper_gatt_print_descriptor(desc_to_init);
                if (add_descr_ret)
                {
    ...
    case ESP_GATTS_ADD_CHAR_DESCR_EVT:
    {
        ...
        s_gatt_service_t *service = esp_ble_wrapper_profile_get_service_by_gatts_if(gatts_if);
        s_gatt_char_t *char_initiating = esp_ble_wrapper_profile_get_initiating_characterstics(service);
        s_gatt_descr_t *desc_initiating = esp_ble_wrapper_profile_get_initiating_descriptor(char_initiating);
        if (desc_initiating)
        {
    ...
    {
        esp_ble_wrapper_gatt_call_read_by_handle(gatts_if, param);
    ...
    {
        esp_ble_wrapper_gatt_call_write_by_handle(gatts_if, param);
        break;
    }
    case ESP_GATTS_EXEC_WRITE_EVT:
    {
        esp_ble_wrapper_gatt_call_write_exec_by_handle(gatts_if, param);
        break;
    }
    ...
    case ESP_GATTS_DISCONNECT_EVT:
    {
        ...
        esp_ble_wrapper_gap_start_advertising();
        break;
    }
    ...
    default:
    {
        TRACE_W("BLE GATT Event: %s Not Implemented!", esp_ble_wrapper_gatt_event_to_string(event));
        break;
    }
    }
}

static f_upcall_t esp_ble_wrapper_gatt_call_by_handle(esp_gatt_if_t gatts_if, uint16_t handle, esp_gatts_cb_event_t event)
{
    s_gatt_service_t *service = esp_ble_wrapper_profile_get_service_by_gatts_if(gatts_if);
    if (service)
    {
        ...
        while (characteristic)
        {
            ...
            {
                // esp_ble_wrapper_gatt_print_characteristic(characteristic);
                switch (event)
                {
                case ESP_GATTS_READ_EVT:
                {
            ...
            while (descriptor)
            {
                if (handle == descriptor->handle)
                {
                    ...
                    // esp_ble_wrapper_gatt_print_descriptor(descriptor);
                    switch (event)
                    {
                    case ESP_GATTS_READ_EVT:
                    {
...
static void esp_ble_wrapper_gatt_call_write_by_handle(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    f_upcall_t write_upcall = esp_ble_wrapper_gatt_call_by_handle(gatts_if, param->write.handle, ESP_GATTS_WRITE_EVT);
    if (write_upcall)
    {
...
static void esp_ble_wrapper_gatt_call_write_exec_by_handle(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    f_upcall_t write_exec_upcall = esp_ble_wrapper_gatt_call_by_handle(gatts_if, param->write.handle, ESP_GATTS_EXEC_WRITE_EVT);
...
static void esp_ble_wrapper_gatt_call_read_by_handle(esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    f_upcall_t read_upcall = esp_ble_wrapper_gatt_call_by_handle(gatts_if, param->read.handle, ESP_GATTS_READ_EVT);
