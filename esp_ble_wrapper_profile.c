...
static void esp_ble_wrapper_gatt_append_descriptor_to_characteristic(s_gatt_char_t *characteristic, s_gatt_descr_t *descriptor);
static void esp_ble_wrapper_gatt_append_characterstic_to_service(s_gatt_service_t *service_obj, s_gatt_char_t *character_object);
static void esp_ble_wrapper_gatt_service_append_to_head(s_gatt_service_t *service_obj);
static s_gatt_service_t *esp_ble_wrapper_gatt_search_service_by_characteristic(s_gatt_char_t *characteristic);
...
s_gatt_service_t *esp_ble_wrapper_gatt_create_service(uint16_t app_id, esp_bt_uuid_t *service_uuid)
{
    ...
    if (service_obj)
    {
        ...
        esp_ble_wrapper_gatt_service_append_to_head(service_obj);
    }
    else
    {
                                                       ...
                                                       f_upcall_t read_func, f_upcall_t write_func, f_upcall_t write_exec_func)
{
    ...
    if (service_obj)
    {
        ...
        if (character_object)
        {
            ...
            esp_ble_wrapper_gatt_append_characterstic_to_service(service_obj, character_object);
                                                    ...
                                                    f_upcall_t read_func, f_upcall_t write_func, f_upcall_t write_exec_func)
{
    ...
    if (charcteristic)
    {
        ...
        if (descriptor_obj)
        {
            ...
            esp_ble_wrapper_gatt_append_descriptor_to_characteristic(charcteristic, descriptor_obj);
            s_gatt_service_t *cur_service = esp_ble_wrapper_gatt_search_service_by_characteristic(charcteristic);
            if (cur_service)
            {
...
// static void esp_ble_wrapper_gatt_print_service(s_gatt_service_t *service);
// static void esp_ble_wrapper_gatt_print_characteristic(s_gatt_char_t *characteristic);
// static void esp_ble_wrapper_gatt_print_descriptor(s_gatt_char_t *descriptor);
// static void esp_ble_wrapper_gatt_print_uuid(esp_bt_uuid_t *uuid, char *msg);
...
void esp_ble_wrapper_profile_print(void)
{
    ...
    while (cur_service)
    {
        esp_ble_wrapper_gatt_print_service(cur_service);

        s_gatt_char_t *cur_character = cur_service->characteristics;
        while (cur_character)
        {
            esp_ble_wrapper_gatt_print_characteristic(cur_character);

            s_gatt_descr_t *cur_descriptor = cur_character->descriptor;
            while (cur_descriptor)
            {
                esp_ble_wrapper_gatt_print_descriptor(cur_descriptor);
                TRACE_B("|    |    |----------------------------------------------------");
                cur_descriptor = cur_descriptor->next;
            }
...
void esp_ble_wrapper_gatt_print_descriptor(s_gatt_descr_t *descriptor)
{
    ...
    {
        TRACE_B("|    |    |--------------Descriptor----------------------------");
        esp_ble_wrapper_gatt_print_uuid(&descriptor->uuid, "|    |    |-");
...
void esp_ble_wrapper_gatt_print_characteristic(s_gatt_char_t *characteristic)
{
    if (characteristic)
    {
        ...
        esp_ble_wrapper_gatt_print_uuid(&characteristic->uuid, "|    |-");
...
{
    TRACE_B("--------------------------Service-------------------------------");
    esp_ble_wrapper_gatt_print_uuid(&service->service_id.id.uuid, "|-");
