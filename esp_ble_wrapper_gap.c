...
static void esp_ble_wrapper_setup_service_uuid(void);
static void show_bonded_devices(void);
static char *esp_key_type_to_str(esp_ble_key_type_t key_type);
static char *esp_auth_req_to_str(esp_ble_auth_req_t auth_req);
static char *esp_ble_wrapper_gap_event_to_str(esp_gap_ble_cb_event_t event);
...
void esp_ble_wrapper_gap_config_adv_data(void)
{
    ...
    {
        esp_ble_wrapper_setup_service_uuid();
    }
...
{
    TRACE_B("BLE GAP Eevent: [%d]-%s", event, esp_ble_wrapper_gap_event_to_str(event));
    switch (event)
    {
    ...
    default:
    {
        TRACE_W("BLE GAP Event: %s Not Implemented!", esp_ble_wrapper_gap_event_to_str(event));
        break;
    }
    }
}
...
static void esp_ble_wrapper_setup_service_uuid(void)
{
    s_gatt_service_t *service_head = esp_ble_wrapper_profile_get_head();
    while (service_head)
    {
    ...
    if (all_service_uuid_len)
    {
        ...
        {
            int uuid_pos = 0;
            service_head = esp_ble_wrapper_profile_get_head();
            while (service_head)
            {
